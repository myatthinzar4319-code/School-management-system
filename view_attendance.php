...
if ($enrolled_course) {
    // Fetch attendance records for the enrolled course
    $stmt = $conn->prepare("SELECT date, status FROM attendance WHERE student_id = ? AND course = ?");
    $stmt->bind_param("is", $student_id, $enrolled_course);
    $stmt->execute();
...
<body>
    ...
    <div class="container">
        <h2>Your Attendance for <?php echo htmlspecialchars($enrolled_course); ?></h2>
        ...
        <?php } else { ?>
            <div class="alert alert-warning mt-3">No attendance records available at the moment.</div>
        <?php } ?>
