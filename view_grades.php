...
if ($enrolled_course) {
    // Fetch grades based on exercises
    $stmt = $conn->prepare("SELECT exercise, grade FROM grades WHERE student_id = ? AND course = ? ORDER BY exercise ASC");
...
<body>
    ...
    <div class="container">
        <h2>Your Grades for <?php echo htmlspecialchars($enrolled_course); ?></h2>
        ...
        <?php } else { ?>
            <div class="alert alert-warning mt-3">No grades available at the moment.</div>
        <?php } ?>
